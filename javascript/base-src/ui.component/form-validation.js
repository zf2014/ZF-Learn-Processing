 ...
 *  @last: 2013-06-03
 */
;(function($, S, undefined) {'use strict';

    var global = S.ENV.host,
        VALIDATION_DATA_CACHE = 'zjp.data.validator',
        VALIDATION_EVENT_NAMESPACE = '.zjp.event.validator',
        VALIDATION_ERRORLIST_CLASSNAME = 'validate-error-list',
        ...
        defaultOptions = {
            ...
            successClass : 'validate-success',  // 验证通过的样式
            errorClass : 'validate-error',      // 验证失败的样式
            errorMessage : false,               // 
            validators : {},                    // 自定义验证规则
            ...
            errors : { 
                ...
                errorsWrapper : '<ul></ul>',
                errorElem : '<li class="form-remind_error"></li>'
            },
            listeners : {
    ...
    ValidateField.prototype = {
        ...
        ulErrorManagement : function() {
            ...
            this.ulTemplate = $(this.options.errors.errorsWrapper).attr('id', this.hash).addClass(VALIDATION_ERRORLIST_CLASSNAME);
        },
    ...
    toggle = function(node, option, fn){
        var $node = $(node),
            newInstance = null,
            options
        ;

        options = S.mix({}, defaultOptions, 'undefined' !== typeof global.ValidateConfig ? global.ValidateConfig : {}, option, node.data());


        // 如果目标元素是form
        if ($node.is('form') || true === $node.data('bind')) {
            newInstance = bind('ValidateForm');
        } else if ($node.is(options.inputs) && !$node.is(options.excluded)) {
        // 如果目标元素字段
            newInstance = bind(!$node.is('input[type=radio], input[type=checkbox]') ? 'ValidateField' : 'ValidateFieldMultiple');
        }
    ...
    // 对外接口设置
    S.namespace('UI').validate = function(node, option, fn){

        var validation = toggle(node, option, fn);
    ...
    $(function(){
        $('.js-FormValidator').each(function() {
            var instance = toggle($(this));
        });
    });
